...
class generateQuery_wfm extends generateQuery_reports {
	...
	static function test() {
		...
		while ($events_row = $db->fetchByAssoc($events_query)) {
			...
			wfm_utils::wfm_log('asol', "\$event_id=[$event_id]", __FILE__, __METHOD__, __LINE__);

			self::getObjectsIds_fromEventId($event_id);
		}
	}

	static function getObjectsIds_fromEventId($eventId, $alternative_database, $trigger_module, $userTZ) {
		...
		//wfm_utils::wfm_log('asol', '$object_ids=['.print_r($object_ids, true).']', __FILE__, __METHOD__, __LINE__);
...
//			$focus->retrieve($eventId);
	...
	static function getQueryArray($wfm_module, $wfm_module_Id, $trigger_module, $userTZ, $alternative_database) {
		wfm_utils::wfm_log('debug', "ENTRY", __FILE__, __METHOD__, __LINE__);
		wfm_utils::wfm_log('debug', 'get_defined_vars=['.var_export(get_defined_vars(), true).']', __FILE__, __METHOD__, __LINE__);
	...
	wfm_utils::wfm_log('asol', 'DESPUES 5', __FILE__, __METHOD__, __LINE__);
		...
		$querys = self::getQueryArray_fromConditions_or_fromCustomVariables($field_values, $filter_values, $trigger_module, $userTZ, $modulesTables, $user_id, $alternative_database);

		return $querys;
	}

	static function getQueryArray_fromConditions_or_fromCustomVariables($field_values, $filter_values, $trigger_module, $userTZ, $modulesTables, $user_id, $alternative_database) {
		wfm_utils::wfm_log('debug', "ENTRY", __FILE__, __METHOD__, __LINE__);
		wfm_utils::wfm_log('debug', 'get_defined_vars=['.var_export(get_defined_vars(), true).']', __FILE__, __METHOD__, __LINE__);
	...
	wfm_utils::wfm_log('asol', 'DESPUES 3', __FILE__, __METHOD__, __LINE__);
		...
		if (($isDomainsInstalled) && ($user_id !== null)) {
			generateQuery_reports::modifySqlWhereForAsolDomainsQuery($sqlWhereQuery, $trigger_module_table, $theUser, true, $theUser->asol_domain_id, $domainField);
		}
