...
function generate_Event_HTML($id, $name, $description, $top, $left, $event_conditions, $type, $trigger_type, $alternative_database, $trigger_event, $scheduled_type, $module) {
	...
	$event_info = generate_event_info_HTML($type, $trigger_type, $alternative_database, $trigger_event, $scheduled_type);
	...
	if ($task_type == 'call_process') {
		...
		$draw_Call_process_open_subprocess .= '
													<span class="task_call_process_open_subprocess_icon">
														<img qtip_info="'.$subprocess_info.'" src="modules/asol_Process/_flowChart/images/task_call_process_open_subprocess_16.png" onclick="window.opener.location=\'index.php?module=asol_Process&action=DetailView&record='.$id_process.'\';" >
													</span>
												';
	}
...
function generate_event_info_HTML($type, $trigger_type, $alternative_database, $trigger_event, $scheduled_type) {
	wfm_utils::wfm_log('debug', 'get_defined_vars=['.var_export(get_defined_vars(), true).']', __FILE__, __METHOD__, __LINE__);
...
if (array_key_exists('events', $export_array)) {
	foreach ($export_array['events'] as $key_parent_process => $value_parent_process) {
		foreach ($value_parent_process as $key_event => $value_event) {
			$draw_Events .= generate_Event_HTML($value_event['id'], $value_event['name'], $value_event['description'], $draw_information_event[$value_event['id']], $left_Process, $value_event['conditions'], $value_event['type'], $value_event['trigger_type'], $export_array['processes'][0]['alternative_database'], $value_event['trigger_event'], $value_event['scheduled_type'], $export_array['processes'][0]['trigger_module']);
			//$top_Process = $top_Process + $height_Event + $separation_vertical_Event;
		}
	}
}
...
$control_panel = '
					<div class="control_panel">
						<button class="nice persistent">'.translate('LBL_ASOL_INFO_TITLE', 'asol_Process').'</button>
