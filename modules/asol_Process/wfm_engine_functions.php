...
function instanciate_processes__scheduled_sequential($scheduled_event_info) {
	...
	instanciate_processes('scheduled', 'sequential', $event_id, $trigger_module, $trigger_event, $object_ids, $current_user_id, $bean_ungreedy_count, $old_bean, $new_bean, $custom_variables, $alternative_database);
}
...
function instanciate_processes__scheduled_parallel($event_id, $alternative_database, $trigger_module, $num_objects, $iter_object, $object_id) {
	...
	instanciate_processes('scheduled', 'parallel', $event_id, $trigger_module, $trigger_event, $object_id, $current_user_id, $bean_ungreedy_count, $old_bean, $new_bean, $custom_variables, $alternative_database);
}
...
function instanciate_processes__logic_hook($trigger_module, $trigger_event, $bean_id, $current_user_id, $bean_ungreedy_count, $old_bean, $new_bean, $server, $request, $current_user_array, $env) {
	...
	instanciate_processes('logic_hook', $scheduled_type, $event_id, $trigger_module, $trigger_event, $bean_id, $current_user_id, $bean_ungreedy_count, $old_bean, $new_bean, $custom_variables, $alternative_database);
}
...
function instanciate_processes($execution_type, $scheduled_type, $event_id, $trigger_module, $trigger_event, $object_ids, $current_user_id, $bean_ungreedy_count, $old_bean, $new_bean, $custom_variables, $alternative_database) {
	...
	switch ($execution_type) {
		...
		case 'logic_hook':
			...
			// asol_domains
			$parent_asol_domain_ids = ($isDomainsInstalled) ? ("," . wfm_reports_utils::wfm_getParentDomains_string($new_bean['asol_domain_id'])) . ",''" : '';
			...
			$event_query = $db->query("
											SELECT asol_events.id, asol_events.name, asol_events.conditions
											FROM asol_events
											...
											WHERE (asol_process.trigger_module = '{$trigger_module}' AND asol_events.trigger_event = '{$trigger_event}' AND asol_events.trigger_type = 'logic_hook' AND asol_events.deleted = 0 {$asol_domains_query_1})							  
									  ");
			$events = Array();
			...
			foreach ($events as $event) {
				wfm_utils::wfm_log('asol', "Check appliesConditions for wfm-event=[name=[{$event['name']}], id=[{$event['id']}]]", __FILE__, __METHOD__, __LINE__);
				if (appliesConditions($trigger_module, $object_ids, $event['conditions'], $current_user_id, $old_bean, $new_bean, $custom_variables, $alternative_database)) {
					wfm_utils::wfm_log('asol', "The wfm-event=[name=[{$event['name']}], id=[{$event['id']}]] Applies conditions=[{$event['conditions']}]", __FILE__, __METHOD__, __LINE__);
					$events_apply[] = $event['id'];
				} else {
					wfm_utils::wfm_log('asol', "The wfm-event=[name=[{$event['name']}], id=[{$event['id']}]] does NOT Applies conditions=[{$event['conditions']}]", __FILE__, __METHOD__, __LINE__);
				}
			}
	...
	wfm_utils::wfm_log('debug', '$events_apply=['.var_export($events_apply, true).']', __FILE__, __METHOD__, __LINE__);
	...
	foreach ($activities as $activity) {
		...
		$activities_info_query = $db->query("
												...
												INNER JOIN asol_proces_asol_events_c ON (asol_proces_asol_events_c.asol_procea8ca_events_idb = asol_events.id AND asol_proces_asol_events_c.deleted = 0)
												INNER JOIN asol_process ON (asol_process.id = asol_proces_asol_events_c.asol_proce6f14process_ida AND asol_process.deleted = 0)
												WHERE asol_activity.id = '{$activity['activity_id']}' AND asol_process.status = 'active'
	...
	wfm_utils::wfm_log('debug', '$activities_apply=['.var_export($activities_apply, true).']', __FILE__, __METHOD__, __LINE__);
	...
	foreach ($activities_apply as $activity) {
		...
		switch ($execution_type) {
				...
				switch ($activity['event_type']) {
					...
					case 'cancel':
						...
						foreach (getArrayProcessInstanceIds($activity['process_id'], $object_ids) as $process_instance_id) {
							...
							$array_InsertQuery[] = "
														INSERT INTO asol_workingnodes (id, name, asol_processinstances_id_c, priority, date_entered, date_modified, asol_events_id_c , asol_activity_id_c, object_ids, iter_object, parent_type, parent_id, asol_task_id_c, delay_wakeup_time, created_by, modified_user_id, assigned_user_id, status, old_bean, new_bean, custom_variables {$asol_domains_query_1})
														VALUES ('{$id2}', '{$name2}', '{$process_instance_id}', {$priority}, '{$date_entered}', '{$date_modified}', '{$event}', '{$current_activity}', '{$object_ids}', {$custom_variables['iter_object']}, '{$trigger_module}', '{$object_id}', {$current_task}, '{$delay_wakeup_time}', '{$created_by}', '{$modified_user_id}', '{$assigned_user_id}', '{$status}', '{$old_bean_to_db}', '{$new_bean_to_db}', '{$custom_variables_to_db}' {$asol_domains_query_2})
												   ";
						}
				}
				break;
			...
			case 'scheduled':
				...
				$status = 'not_started';
				// Set priority
				$priority = $app_list_strings['wfm_working_node_priority'][$activity['event_trigger_type']];
				$old_bean_to_db = '';
				$new_bean_to_db = base64_encode(serialize($new_bean));
				$custom_variables_to_db = base64_encode(serialize($custom_variables));
...
function getActivities_appliesCondition($activities_info, $trigger_module, $bean_id, $current_user_id, $old_bean, $new_bean, $custom_variables, $alternative_database) {
	...
	foreach ($activities_info as $activity_info) {
		wfm_utils::wfm_log('asol', "Check appliesConditions for wfm-activity=[name=[{$activity_info['activity_name']}], id=[{$activity_info['activity_id']}]]", __FILE__, __METHOD__, __LINE__);
		if (appliesConditions($trigger_module, $bean_id, $activity_info['activity_conditions'], $current_user_id, $old_bean, $new_bean, $custom_variables, $alternative_database)) {
			wfm_utils::wfm_log('asol', "The wfm-activity=[name=[{$activity_info['activity_name']}], id=[{$activity_info['activity_id']}]] Applies conditions=[{$activity_info['activity_conditions']}]", __FILE__, __METHOD__, __LINE__);
			$activities_apply[] = $activity_info;
		} else {
			wfm_utils::wfm_log('asol', "The wfm-activity=[name=[{$activity_info['activity_name']}], id=[{$activity_info['activity_id']}]] does NOT Applies conditions=[{$activity_info['activity_conditions']}]", __FILE__, __METHOD__, __LINE__);
		}
	}
...
function getArrayProcessInstanceIds($process_id, $bean_id) {
	...
	$sql = "
			...
			INNER JOIN asol_working_nodes ON asol_processinstances.id = asol_working_nodes.asol_processinstances_id_c
			WHERE asol_processinstances.asol_process_id_c = '{$process_id}' AND asol_working_nodes.bean_id = '{$bean_id}'
			";
...
function continue_wfm_execution_through_curl() {
	...
	$log = "\$MAX_working_nodes_executed_in_one_php_instance reached! => Continue through cURL";
...
function execute_WFM() {
	wfm_utils::wfm_log('asol', "ENTRY", __FILE__, __METHOD__, __LINE__);

	global $db/*, $current_user*/, $sugar_config;

	$execution_counter = 0;
	$MAX_working_nodes_executed_in_one_php_instance = (isset($sugar_config['MAX_working_nodes_executed_in_one_php_instance'])) ? $sugar_config['MAX_working_nodes_executed_in_one_php_instance'] : 10;

	do {

		if (($MAX_working_nodes_executed_in_one_php_instance != 'unlimited') && ($execution_counter >= $MAX_working_nodes_executed_in_one_php_instance)) {
		...
		$working_node_query = $db->query("
											SELECT *
											FROM asol_workingnodes
											WHERE (   status = 'not_started' OR  status = 'in_progress' OR ( (status = 'delayed_by_activity' OR status = 'delayed_by_task') AND delay_wakeup_time <= '{$delay_wakeup_time}')  )
											ORDER BY priority ASC, date_modified DESC
											LIMIT 1
										"); // status=executing means that a php-script is already executing this working_node, so do NOT execute it again
	...
	wfm_utils::wfm_log('info', $log, __FILE__, __METHOD__, __LINE__);
...
function execute_working_node($process_instance, $working_node) {
	...
	if ((empty($working_node['asol_task_id_c'])) && ($activityDelay) && ($working_node['status'] != 'delayed_by_activity') && ($working_node['status'] != 'delayed_by_task')) {//////***** si la task es null AND si hay delay para la current_activity AND estado del nodo != delayed
		...
		if ($scheduled_type == 'sequential') {
			...
			while ($more_objects_to_check_appliesConditions) {
				wfm_utils::wfm_log('asol', "Check appliesConditions for wfm-activity=[name=[{$activity_row['name']}], id=[{$activity_row['id']}]]", __FILE__, __METHOD__, __LINE__);
				...
				if ($appliesConditions_flag) {
					wfm_utils::wfm_log('asol', "The wfm-activity=[name=[{$activity_row['name']}], id=[{$activity_row['id']}]] Applies conditions=[{$activity_row['conditions']}]", __FILE__, __METHOD__, __LINE__);
					break;
				} else {
					wfm_utils::wfm_log('asol', "The wfm-activity=[name=[{$activity_row['name']}], id=[{$activity_row['id']}]] does NOT Applies conditions=[{$activity_row['conditions']}]", __FILE__, __METHOD__, __LINE__);
				}
		...
		if ($appliesConditions_flag) {
			...
			$tasksQuery = $db->query("
										...
										ON asol_activity_asol_task_c.asol_activf613ol_task_idb = asol_task.id AND asol_activity_asol_task_c.deleted = 0
										WHERE asol_activity_asol_task_c.asol_activ5b86ctivity_ida = '{$working_node['asol_activity_id_c']}' AND asol_task.deleted = 0
										ORDER BY asol_task.task_order
									 ");
			$tasks = Array();
			...
			foreach ($tasks as $keyT => $task) {
				...
				if (!$taskDelayStartTime) { // If current wfm-task has no delay, execute it
					...
					if (     (($task['task_type'] != 'modify_object') && ($task['task_type'] != 'call_process') && ($process_instance['asol_processinstances_id_c'] != null))      || ($process_instance['asol_processinstances_id_c'] == null) ) {
						...
						wfm_utils::wfm_log('asol', '$executeResult=['.var_export($executeResult,true).']'." for wfm-task=[name=[{$task['name']}], id=[{$task['id']}]]", __FILE__, __METHOD__, __LINE__);
						...
						if ($task['task_type'] == "end") { // if task_type=end then break the foreach execution in order to not execute the following tasks (there should not be following tasks)
	...
	// CLEAN DATABASE
	cleanDatabase($process_instance['id']);
...
function updateIterObject($process_instance, &$working_node, $alternative_database, $trigger_module, &$new_bean, &$custom_variables, $remove_current_object) {
	...
	wfm_utils::wfm_log('debug', '$working_node[\'iter_object\']=['.var_export($working_node['iter_object'], true).']', __FILE__, __METHOD__, __LINE__);
...
function cleanDatabase($process_instance_id) {
	...
	$number_of_active_working_nodes__query_2 = $db->query("
																...
																FROM asol_workingnodes
																WHERE status != 'terminated' AND asol_processinstances_id_c = '{$process_instance_id}'
														");
...
function manageWorkingNodes($task, $next_activities, $nextTask, $process_instance, $working_node, $executeResult, $current_user_id, $old_bean, $new_bean, $custom_variables) {
	...
	if (count($next_activities) >= 2) { // This branch ends and two or more branchs are borne from the branch that is going to die.
		...
		wfm_utils::wfm_log('debug', "count(next_activities)>=2", __FILE__, __METHOD__, __LINE__);
		wfm_utils::wfm_log('debug', "This branch ends and two or more branchs are borne from the branch that is going to die", __FILE__, __METHOD__, __LINE__);
		...
		foreach ($next_activities as $next_activity_id) {
			...
			$db->query("
							INSERT INTO asol_workingnodes (id, name, asol_processinstances_id_c, priority, date_entered, date_modified, asol_events_id_c, asol_activity_id_c, object_ids, iter_object, parent_type, parent_id, asol_task_id_c, delay_wakeup_time, created_by, modified_user_id, assigned_user_id, status, old_bean, new_bean, custom_variables {$asol_domains_query_1})
							VALUES ('{$id3}', '{$name3}', '{$process_instance['id']}', '{$working_node['priority']}', '{$date_entered}', '{$date_modified}', '{$event}', '{$next_activity_id}', '{$object_ids}', {$iter_object}, '{$trigger_module}', '{$object_id}', {$current_task}, '{$delay_wakeup_time}', '{$created_by}', '{$modified_user_id}', '{$assigned_user_id}', '{$status}', '{$old_bean_to_db}', '{$new_bean_to_db}', '{$custom_variables_to_db}' {$asol_domains_query_2})
					   ");
		}
	} elseif (count($next_activities) == 1) { // Follow with the same branch

		wfm_utils::wfm_log('debug', "count(next_activities)==1", __FILE__, __METHOD__, __LINE__);
		wfm_utils::wfm_log('debug', "Follow with the same branch", __FILE__, __METHOD__, __LINE__);
		...
		$db->query("
						UPDATE asol_workingnodes 
						SET custom_variables = '{$custom_variables_to_db}' 
						WHERE id = '{$working_node['id']}'
				   ");
		...
		wfm_utils::wfm_log('debug', "count(next_activities)==0", __FILE__, __METHOD__, __LINE__);
		wfm_utils::wfm_log('debug', "There is no next_activity in this branch, the branch dies with no children", __FILE__, __METHOD__, __LINE__);
...
function appliesConditions($trigger_module, $bean_id, $conditions, $current_user_id, $old_bean, $new_bean, $custom_variables, $alternative_database) { //Returns true or false if the given conditions applies for the module bean_id
	...
	wfm_utils::wfm_log('debug', "final after foreach \$array_appliesCurrentCondition=[".var_export($array_appliesCurrentCondition,true)."] \n\n\n", __FILE__, __METHOD__, __LINE__);
	wfm_utils::wfm_log('debug', "final after foreach \$array_parenthesis=[".var_export($array_parenthesis,true)."] \n\n\n", __FILE__, __METHOD__, __LINE__);
	wfm_utils::wfm_log('debug', "final after foreach \$array_logicalOperator=[".var_export($array_logicalOperator,true)."] \n\n\n", __FILE__, __METHOD__, __LINE__);
	...
	wfm_utils::wfm_log('debug', "final after foreach  \$appliesConditions=[".var_export($appliesConditions,true)."] ", __FILE__, __METHOD__, __LINE__);
