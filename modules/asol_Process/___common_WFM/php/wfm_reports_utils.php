...
class wfm_reports_utils {
	...
	static function wfm_manageExternalDatabaseQueries($alternativeDb, $reportModule) {
		...
		if ($alternativeDb !== false) {
			...
			if (isset($sugar_config['asolReportsAlternativeDbConnections'][$alternativeDb]['asolSpecificTableDomainIdField'][$report_table])) {
				$domainField = $sugar_config['asolReportsAlternativeDbConnections'][$alternativeDb]['asolSpecificTableDomainIdField'][$report_table];
			} else if (isset($sugar_config['asolReportsAlternativeDbConnections'][$alternativeDb]['asolDefaultDbDomainIdField'])) {
				$domainField = $sugar_config['asolReportsAlternativeDbConnections'][$alternativeDb]['asolDefaultDbDomainIdField'];
			} else {
				$domainField = "";
			}
	 ...
	 * D20130607T1818
	 * Get parent domain
	 * @param $asol_domain_id
	 */
	static function wfm_getParentDomain($asol_domain_id) {

		global $db;

		$parent_domain_query = $db->query("
										SELECT asol_domains_id_c
										FROM asol_domains
										WHERE id = '{$asol_domain_id}' AND deleted = 0							
									");
		$parent_domain_row = $db->fetchByAssoc($parent_domain_query);
		$parent_asol_domain_id = $parent_domain_row['asol_domains_id_c'];

		return $parent_asol_domain_id;
	}
	 ...
	 * @param $childs
	 */
	static function getChildDomains($domainId, &$childs=array()){

		global $db;

		$child_query = $db->query("SELECT id, asol_domain_enabled FROM asol_domains WHERE asol_domains_id_c='".$domainId."' AND deleted=0 ORDER BY name ASC");

		while($queryChildRow = $db->fetchByAssoc($child_query)) {
			$childs[count($childs)]['id'] = $queryChildRow['id'];
			$childs[count($childs)-1]['enabled'] = $queryChildRow['asol_domain_enabled'];
			$this->getChildDomains($queryChildRow['id'], $childs);
		}
	...
	/**
	 * D20130607T1818
	 * Get parent domains with height(self=0, father=1, grandfather=2, etc)
	 * @param $domainId
	 * @param $addSelfDomain
	 */
	static function getParentDomainsWithHeight($domainId, $addSelfDomain = false){
		...
		if ($addSelfDomain) {
			$parents[$domainId]['id'] = $domainId;
			$parents[$domainId]['height'] = 0;
		}


		$parent_query = $db->query("SELECT asol_domains_id_c FROM asol_domains WHERE id='".$domainId."' AND deleted=0 ORDER BY name ASC");
		$parent_row = $db->fetchByAssoc($parent_query);

		$i=1;

		while(!empty($parent_row["asol_domains_id_c"])) {

			$parents[$parent_row["asol_domains_id_c"]]['id'] = $parent_row["asol_domains_id_c"];
			$parents[$parent_row["asol_domains_id_c"]]['height'] = $i;
			$parent_query = $db->query("SELECT asol_domains_id_c FROM asol_domains WHERE id='".$parent_row["asol_domains_id_c"]."' AND deleted=0 ORDER BY name ASC");
			$parent_row = $db->fetchByAssoc($parent_query);
	...
	static function wfm_getChildDomains_string($domainId) {
		...
		$childDomains = getChildDomainsWithDepth($domainId);
	...
	static function wfm_getParentDomains_string($domainId) {
		...
		$parentDomains = self::getParentDomainsWithHeight($domainId);
	...
	/**
	 * D20130607T1818
	 * Get parent domains with height(child=1, grandchild=2, etc)
	 * @param $domainId
	 */
	static function getChildDomainsWithDepth($domainId){

		global $db;

		$childs = getChildDomains($domainId);

		$hierarchyChilds = Array();

		foreach ($childs as $child) {

			$domainQuery = $db->query("SELECT asol_domain_enabled, asol_domains_id_c FROM asol_domains WHERE id='".$child["id"]."' ORDER BY name ASC");
			$domainRow = $db->fetchByAssoc($domainQuery);

			$hierarchyChilds[count($hierarchyChilds)]["id"] = $child["id"];
			$hierarchyChilds[count($hierarchyChilds)-1]['enabled'] = $domainRow['asol_domain_enabled'];

			$depth = 1;
			while ((!empty($domainRow['asol_domains_id_c'])) && ($domainRow['asol_domains_id_c'] != $domainId)) {
				$domainQuery = $db->query("SELECT asol_domain_enabled, asol_domains_id_c FROM asol_domains WHERE id='".$domainRow['asol_domains_id_c']."' ORDER BY name ASC");
				$domainRow = $db->fetchByAssoc($domainQuery);
				$depth++;
			}

			$hierarchyChilds[count($hierarchyChilds)-1]["depth"] = $depth;
	...
	static public function managePremiumFeature($premiumFeature, $requiredFile, $callFunction, $extraParams, $isJsFile = false) {
		...
		if (!file_exists($basePremiumPath.$requiredFile)) {
			...
			if (!$isJsFile)
			wfm_utils::wfm_log('warn', "Cannot get ".$premiumFeature." Premium Feature. ".$callFunction."() Function Called.", __FILE__, __METHOD__, __LINE__);
			else
			wfm_utils::wfm_log('warn', "Cannot get ".$premiumFeature." Premium Feature. Tried to Load '".$requiredFile."' File", __FILE__, __METHOD__, __LINE__);
			return false;
