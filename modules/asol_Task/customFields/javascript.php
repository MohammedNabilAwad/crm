...
$systemUsersAndRoles = getSystemUsersAndRoles(wfm_reports_utils::wfm_isDomainsInstalled());
$users_opts = $systemUsersAndRoles['users'];
$acl_roles_opts = $systemUsersAndRoles['roles'];

function getSystemUsersAndRoles($isDomainsInstalled) {
	...
	if ($isDomainsInstalled) {
		$users_sql .= "SELECT id, user_name FROM users WHERE deleted=0 AND asol_domain_id='".$current_user->asol_default_domain."'";
	} else {
...
<link rel='stylesheet' href='modules/asol_Process/___common_WFM/plugins_js_css_images/codemirror_php/codemirror.css'>
...
<style type='text/css'>.CodeMirror {border-top: 1px solid black; border-bottom: 1px solid black; min-width: 400px; max-width: 400px;} .CodeMirror-gutters { background-color: wheat;}</style>
...
<script>
	...
	function main() {
		...
		$(document).ready(function() {
			...
			generate_taskImplementation_default_noData('task_type', asol_var['focus']['task_type'], asol_var['users_opts'], asol_var['acl_roles_opts'], asol_var['calendar_dateformat']);
	...
	function rewriteOnClickCode_saveButton() {
		...
		$("#EditView .primary").each(function() {
			...
			var new_onclickCode = "save_php_custom(); document.getElementById('task_implementation_hidden').value=format_taskImplementation('task_type');" + onclickCode;
	...
	function onChange_task_type(dropdown) {
		...
		switch (dropdown.value) {
			...
			case 'modify_object':
				dropdown.form.action.value = "EditView"; 
			...
			default:
				...
				generate_taskImplementation_default_noData("task_type", null, asol_var['users_opts'], asol_var['acl_roles_opts'], asol_var['calendar_dateformat']); 
