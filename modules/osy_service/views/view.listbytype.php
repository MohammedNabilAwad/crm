...
class osy_serviceViewListbytype extends ViewList {
    ...
    function osy_serviceViewListbytype() {
        parent::ViewList();
    }
 	...
 	function listViewProcess(){
        ...
        $this->params['orderBy'] = 'osy_service.date_start';
    ...
    function processSearchForm(){
         ...
         $two_hours_ago = $GLOBALS['db']->convert($GLOBALS['db']->quoted($timedate->asDb($timedate->getNow()->get("-2 hours"))), 'datetime');
   		...
   		$where =  " osy_service.type = '$type' AND osy_service.status != 'Held' AND osy_service.status != 'Not Held' AND osy_service.date_start > {$two_hours_ago} AND ( osy_service.assigned_user_id = '".$GLOBALS['db']->quote($GLOBALS['current_user']->id)."' OR exists ( SELECT id FROM osy_service_users WHERE osy_service_id = osy_service.id AND user_id = '".$GLOBALS['db']->quote($GLOBALS['current_user']->id)."' AND deleted = 0 ) ) ";

          if ( isset($_REQUEST['name_basic']) ) {
              $name_search = trim($_REQUEST['name_basic']);
              if ( ! empty($name_search) ) {
                  $where .= " AND osy_service.name LIKE '".$GLOBALS['db']->quote($name_search)."%' ";
              }
          }
