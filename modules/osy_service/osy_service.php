...
class osy_service extends SugarBean {
	// Stored fields
	var $id;
	var $date_entered;
	var $date_modified;
	...
	var $table_name = "osy_service";
	var $rel_users_table = "osy_service_users";
	var $rel_contacts_table = "osy_service_contacts";
	var $rel_leads_table = "osy_service_leads";
	var $module_dir = "osy_service";
	var $object_name = "osy_service";
	...
	var $relationship_fields = array('account_id'=>'accounts','opportunity_id'=>'opportunity','case_id'=>'case',
									 'assigned_user_id'=>'users','contact_id'=>'contacts', 'user_id'=>'users', 'osy_service_id'=>'osy_service');
	 ...
	 * sole constructor
	 */
	function osy_service() {
		parent::SugarBean();
		$this->setupCustomFields('osy_service');
...
//		$this->fill_in_additional_detail_fields();
	...
	/**
	 * Disable edit if osy_service is recurring and source is not Sugar. It should be edited only from Outlook.
	 * @param $view string
	 * @param $is_owner bool
	 */
	function ACLAccess($view,$is_owner = 'not_set',$in_group = 'not_set'){
		// don't check if osy_service is being synced from Outlook
	 ...
	 * @return bool
	 */
	function hasIntegratedosy_service() {
		return false;
	}
	...
	function save($check_notify = FALSE) {
		...
		// prevent a mass mailing for recurring osy_service created in Calendar module
        ...
        if ( isset($api) && is_a($api,'Webosy_service') && empty($this->in_relationship_update) ) {
            // Make sure the API initialized and it supports Web osy_service
            // Also make suer we have an ID, the external site needs something to reference
            if ( !isset($this->id) || empty($this->id) ) {
                $this->id = create_guid();
                $this->new_with_id = true;
            }
            $response = $api->scheduleosy_service($this);
    ...
    function create_export_query($order_by, $where)
    {
		...
		if($contact_required) {
			$query = "SELECT osy_service.*, contacts.first_name, contacts.last_name, contacts.assigned_user_id contact_name_owner, users.user_name as assigned_user_name   ";
			if($custom_join) {
				$query .= $custom_join['select'];
			}
			$query .= " FROM contacts, osy_service, osy_service_contacts ";
			$where_auto = " osy_service_contacts.contact_id = contacts.id AND osy_service_contacts.osy_service_id = osy_service.id AND osy_service.deleted=0 AND contacts.deleted=0";
		} else {
			$query = 'SELECT osy_service.*, users.user_name as assigned_user_name  ';
			if($custom_join) {
				$query .= $custom_join['select'];
			}
			$query .= ' FROM osy_service ';
			$where_auto = "osy_service.deleted=0";
		}
		$query .= "  LEFT JOIN users ON osy_service.assigned_user_id=users.id ";
	...
	function fill_in_additional_detail_fields() {
		...
		if(!empty($this->date_start)) {
		    ...
		    if (!empty($start)) {
		        ...
		        $GLOBALS['log']->fatal("osy_service::save: Bad date {$this->date_start} for format ".$GLOBALS['timedate']->get_date_time_format());
		    }
		}
	...
	function get_list_view_data() {
		$osy_service_fields = $this->get_list_view_array();

		global $app_list_strings, $focus, $action, $currentModule;
		if(isset($this->parent_type))
			$osy_service_fields['PARENT_MODULE'] = $this->parent_type;
		if($this->status == "Planned") {
			//cn: added this if() to deal with sequential Closes in osy_service.	this is a hack to a hack(formbase.php->handleRedirect)
			if(empty($action))
			     $action = "index";
            $setCompleteUrl = "<a id='{$this->id}' onclick='SUGAR.util.closeActivityPanel.show(\"{$this->module_dir}\",\"{$this->id}\",\"Held\",\"listview\",\"1\");'>";
			if ($this->ACLAccess('edit')) {
                $osy_service_fields['SET_COMPLETE'] = $setCompleteUrl . SugarThemeRegistry::current()->getImage("close_inline"," border='0'",null,null,'.gif',translate('LBL_CLOSEINLINE'))."</a>";
            } else {
                $osy_service_fields['SET_COMPLETE'] = '';
            }
		}
		global $timedate;
		$today = $timedate->nowDb();
		$nextday = $timedate->asDbDate($timedate->getNow()->get("+1 day"));
		$mergeTime = $osy_service_fields['DATE_START']; //$timedate->merge_date_time($osy_service_fields['DATE_START'], $osy_service_fields['TIME_START']);
		$date_db = $timedate->to_db($mergeTime);
		if($date_db	< $today	) {
			$osy_service_fields['DATE_START']= "<font class='overdueTask'>".$osy_service_fields['DATE_START']."</font>";
		}else if($date_db	< $nextday) {
			$osy_service_fields['DATE_START'] = "<font class='todaysTask'>".$osy_service_fields['DATE_START']."</font>";
		} else {
			$osy_service_fields['DATE_START'] = "<font class='futureTask'>".$osy_service_fields['DATE_START']."</font>";
		}
		$this->fill_in_additional_detail_fields();
        ...
        $osy_service_fields['CONTACT_ID'] = $this->contact_id;
        $osy_service_fields['CONTACT_NAME'] = $this->contact_name;
		$osy_service_fields['PARENT_NAME'] = $this->parent_name;
        $osy_service_fields['REMINDER_CHECKED'] = $this->reminder_time==-1 ? false : true;
        $osy_service_fields['EMAIL_REMINDER_CHECKED'] = $this->email_reminder_time==-1 ? false : true;


		return $osy_service_fields;
	}

	function set_notification_body($xtpl, &$osy_service) {
		global $sugar_config;
		global $app_list_strings;
		global $current_user;
		global $timedate;
		...
		$notifyUser =($osy_service->current_notify_user->object_name == 'User') ? $osy_service->current_notify_user : $current_user;
		// cn: bug 8078 - fixed call to $timedate
		if(strtolower(get_class($osy_service->current_notify_user)) == 'contact') {
			$xtpl->assign("ACCEPT_URL", $sugar_config['site_url'].
							'/index.php?entryPoint=acceptDecline&module=osy_service&contact_id='.$osy_service->current_notify_user->id.'&record='.$osy_service->id);
		} elseif(strtolower(get_class($osy_service->current_notify_user)) == 'lead') {
			$xtpl->assign("ACCEPT_URL", $sugar_config['site_url'].
							'/index.php?entryPoint=acceptDecline&module=osy_service&lead_id='.$osy_service->current_notify_user->id.'&record='.$osy_service->id);
		} else {
			$xtpl->assign("ACCEPT_URL", $sugar_config['site_url'].
							'/index.php?entryPoint=acceptDecline&module=osy_service&user_id='.$osy_service->current_notify_user->id.'&record='.$osy_service->id);
		}
		$xtpl->assign("osy_service_TO", $osy_service->current_notify_user->new_assigned_user_name);
		$xtpl->assign("osy_service_SUBJECT", trim($osy_service->name));
		$xtpl->assign("osy_service_STATUS",(isset($osy_service->status)? $app_list_strings['osy_service_status_dom'][$osy_service->status]:""));
		$typekey = strtolower($osy_service->type);
		if(isset($osy_service->type)) {
		    if(!empty($app_list_strings['eapm_list'][$typekey])) {
    		    $typestring = $app_list_strings['eapm_list'][$typekey];
	    	} else {
		        $typestring = $app_list_strings['osy_service_type_dom'][$osy_service->type];
		    }
		}
		$xtpl->assign("osy_service_TYPE", isset($osy_service->type)? $typestring:"");
		$startdate = $timedate->fromDb($osy_service->date_start);
		$xtpl->assign("osy_service_STARTDATE", $timedate->asUser($startdate, $notifyUser)." ".TimeDate::userTimezoneSuffix($startdate, $notifyUser));
		$enddate = $timedate->fromDb($osy_service->date_end);
		$xtpl->assign("osy_service_ENDDATE", $timedate->asUser($enddate, $notifyUser)." ".TimeDate::userTimezoneSuffix($enddate, $notifyUser));
		$xtpl->assign("osy_service_HOURS", $osy_service->duration_hours);
		$xtpl->assign("osy_service_MINUTES", $osy_service->duration_minutes);
		$xtpl->assign("osy_service_DESCRIPTION", $osy_service->description);
        if ( !empty($osy_service->join_url) ) {
            $xtpl->assign('osy_service_URL', $osy_service->join_url);
            $xtpl->parse('osy_service.osy_service_External_API');
        }
	...
	public function create_notification_email($notify_user){
		...
		//OPENSYMBOLMOD paolo.santacatterina (13/mar/2015  11:42:45)
		//PITCILOCRM-2
		//************************************************************
...
// 			$notify_mail->AddAttachment($path, 'osy_service.ics', 'base64', 'text/calendar');
// 		}
		//************************************************************
		return $notify_mail;
	}
	...
	function get_osy_service_users() {
		$template = new User();
		// First, get the list of IDs.
		$query = "SELECT osy_service_users.required, osy_service_users.accept_status, osy_service_users.user_id from osy_service_users where osy_service_users.osy_service_id='$this->id' AND osy_service_users.deleted=0";
	...
	function get_invite_osy_service(&$user) {
		$template = $this;
		// First, get the list of IDs.
		$GLOBALS['log']->debug("Finding linked records $this->object_name: ".$query);
		$query = "SELECT osy_service_users.required, osy_service_users.accept_status, osy_service_users.osy_service_id from osy_service_users where osy_service_users.user_id='$user->id' AND( osy_service_users.accept_status IS NULL OR	osy_service_users.accept_status='none') AND osy_service_users.deleted=0";
		$result = $this->db->query($query, true);
		$list = Array();

		while($row = $this->db->fetchByAssoc($result)) {
			$record = $template->retrieve($row['osy_service_id']);
			$template->required = $row['required'];
			$template->accept_status = $row['accept_status'];
	...
	function set_accept_status(&$user,$status)
	{
		...
		{
			$relate_values = array('user_id'=>$user->id,'osy_service_id'=>$this->id);
		...
		{
			$relate_values = array('contact_id'=>$user->id,'osy_service_id'=>$this->id);
			$data_values = array('accept_status'=>$status);
			$this->set_relationship($this->rel_contacts_table, $relate_values, true, true,$data_values);
		}
        else if($user->object_name == 'Lead')
		{
			$relate_values = array('lead_id'=>$user->id,'osy_service_id'=>$this->id);
			$data_values = array('accept_status'=>$status);
	...
	function save_relationship_changes($is_update, $exclude = array()) {
	    ...
	    if(empty($this->in_workflow)) {
           if(empty($this->in_import)){//if a osy_service is being imported then contact_id  should not be excluded
           ...
           //add assigned_user_id to exclude list and let the logic from osy_serviceFormBase determine whether assigned user id gets added to the relationship
...
function getosy_serviceExternalApiDropDown($focus = null, $name = null, $value = null, $view = null)
{
	global $dictionary, $app_list_strings;

	$cacheKeyName = 'osy_service_type_drop_down';
    ...
    if ($apiList === null)
    {
        ...
        $apiList = ExternalAPIFactory::getModuleDropDown('osy_service');
    ...
    if ($dictionary['osy_service']['fields']['type']['options'] != "eapm_list")
    {
        $apiList = array_merge(getosy_serviceTypeOptions($dictionary, $app_list_strings), $apiList);
    }
...
/**
 * osy_service Type Options Array for dropdown list
 * @param array $dictionary - getting type name
 * @param array $app_list_strings - getting type options
 * @return array osy_service Type Options Array for dropdown list
 */
function getosy_serviceTypeOptions($dictionary, $app_list_strings)
{
	$result = array();

    // getting name of osy_service type to fill dropdown list by its values
    if (isset($dictionary['osy_service']['fields']['type']['options']))
	{
    	$typeName = $dictionary['osy_service']['fields']['type']['options'];
