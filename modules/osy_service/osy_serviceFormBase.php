 ...
 * Description:  Base Form For osy_service
...
class osy_serviceFormBase extends FormBase {

	function getFormBody($prefix, $mod='', $formname=''){
		if(!ACLController::checkAccess('osy_service', 'edit', true)){
		return '';
	}
		global $mod_strings;
		global $app_strings;
...
$default_status = $app_list_strings['osy_service_status_default'];
...
$javascript->setSugarBean(new osy_service());
...
function getForm($prefix, $mod='osy_service'){
	if(!ACLController::checkAccess('osy_service', 'edit', true)){
		return '';
	}
...
$the_form .= <<<EOQ
		...
		<form name="${prefix}osy_serviceave" onSubmit="return check_form('${prefix}osy_serviceave')" method="POST" action="index.php">
			<input type="hidden" name="${prefix}module" value="osy_service">

			<input type="hidden" name="${prefix}action" value="Save">

EOQ;
$the_form	.= $this->getFormBody($prefix, 'osy_service',"{$prefix}osy_serviceave" );
$the_form .= <<<EOQ
...
/**
 * handles save functionality for osy_service
 * @param	string prefix
...
function handleSave($prefix,$redirect=true, $useRequired=false) {
	...
	//add assigned user and current user if this is the first time bean is saved
  	if(empty($focus->id) && !empty($_REQUEST['return_module']) && $_REQUEST['return_module'] =='osy_service' && !empty($_REQUEST['return_action']) && $_REQUEST['return_action'] =='DetailView'){
		//if return action is set to detail view and return module to osy_service, then this is from the long form, do not add the assigned user (only the current user)
		//The current user is already added to UI and we want to give the current user the option of opting out of osy_service.
	...
	if( (isset($_POST['isSaveFromDetailView']) && $_POST['isSaveFromDetailView'] == 'true') ||
		...
		if($focus->status == 'Held' && $this->isEmptyReturnModuleAndAction() && !$this->isSaveFromDCMenu()){
    		//if we are closing the osy_service, and the request does not have a return module AND return action set and it is not a save
	    	...
	    	// Get all users for the osy_service
	    	$q = 'SELECT mu.user_id, mu.accept_status FROM osy_service_users mu WHERE mu.osy_service_id = \''.$focus->id.'\'';
	    	$r = $focus->db->query($q);
	    	...
	    	if(count($deleteUsers) > 0) {
	    		...
	    		$sql = "UPDATE osy_service_users set deleted = 1 where user_id in ($sql) AND osy_service_id = '". $focus->id . "'";
	    		$focus->db->query($sql);
	    	}

	        // Get all contacts for the osy_service
	    	...
	    	$q = 'SELECT mu.contact_id, mu.accept_status FROM osy_service_contacts mu WHERE mu.osy_service_id = \''.$focus->id.'\'';
	    	$r = $focus->db->query($q);
	    	...
	    	if(count($deleteContacts) > 0) {
	    		...
	    		$sql = substr($sql, 1);
	    		// We could run a delete SQL statement here, but will just mark as deleted instead
	    		$sql = "UPDATE osy_service_contacts set deleted = 1 where contact_id in ($sql) AND osy_service_id = '". $focus->id . "'";
	    		$focus->db->query($sql);
	    	}
	    	...
	    	$q = 'SELECT mu.lead_id, mu.accept_status FROM osy_service_leads mu WHERE mu.osy_service_id = \''.$focus->id.'\'';
	    	$r = $focus->db->query($q);
	    	...
	    	if(count($deleteLeads) > 0) {
	    		...
	    		$sql = substr($sql, 1);
	    		// We could run a delete SQL statement here, but will just mark as deleted instead
	    		$sql = "UPDATE osy_service_leads set deleted = 1 where lead_id in ($sql) AND osy_service_id = '". $focus->id . "'";
	    		$focus->db->query($sql);
	    	}
	    	////	END REMOVE
	    	///////////////////////////////////////////////////////////////////////////
	    	...
	    	// Call the osy_service module's save function to handle saving other fields besides
	    	...
	    	foreach($focus->users_arr as $user_id) {
	    		...
	    		if(!isset($acceptStatusUsers[$user_id])) {
	    			...
	    			$qU  = 'UPDATE osy_service_users SET deleted = 0, accept_status = \''.$acceptStatusUsers[$user_id].'\' ';
	    			$qU .= 'WHERE osy_service_id = \''.$focus->id.'\' ';
	    	...
	    	foreach($focus->contacts_arr as $contact_id) {
	    		...
	    		if(!isset($acceptStatusContacts[$contact_id])) {
	    			...
	    			// update query to preserve accept_status
	    			$qU  = 'UPDATE osy_service_contacts SET deleted = 0, accept_status = \''.$acceptStatusContacts[$contact_id].'\' ';
	    			$qU .= 'WHERE osy_service_id = \''.$focus->id.'\' ';
	    	...
	    	foreach($focus->leads_arr as $lead_id) {
	    		...
	    		if(!isset($acceptStatusLeads[$lead_id])) {
	    			...
	    			// update query to preserve accept_status
	    			$qU  = 'UPDATE osy_service_leads SET deleted = 0, accept_status = \''.$acceptStatusLeads[$lead_id].'\' ';
	    			$qU .= 'WHERE osy_service_id = \''.$focus->id.'\' ';
	...
	if (isset($_REQUEST['return_module']) && $_REQUEST['return_module'] == 'Home'){
		...
		handleRedirect($return_id, 'osy_service');
	} else {
		return $focus;
	}
