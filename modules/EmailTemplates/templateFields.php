    ...
    foreach ($loopControl as $collectionKey => $beans) {
        ...
        foreach ($beans as $beankey => $bean) {
            foreach ($bean->field_defs as $key => $field_def) {
                ...
                if (isset($field_def['vname'])) {
                    /*OPENSYMBOLMOD marco.zarantonello 22/04/21 START*/
                    global $current_language;
                    $optionLabel = preg_replace('/:$/', "", translate($field_def['vname'], $beankey, '', $current_language));
                    $optionLabel = mb_convert_encoding($optionLabel, 'UTF-8', 'UTF-8');
                    /*OPENSYMBOLMOD marco.zarantonello 22/04/21 END*/
                } else {
