...
if (isset($_REQUEST['current_post']) && $_REQUEST['current_post'] != '') {
    ...
    {
        $limit = count($recordIds);

        // if $limit == 1 the action starts from detailview, otherwisein in the listview
        if ($limit > 1)
            $query = $_SESSION["Accounts2_QUERY"]." LIMIT ".$limit;
        else
            $query = "SELECT id from accounts where id = '".$_REQUEST['uid']."'";
