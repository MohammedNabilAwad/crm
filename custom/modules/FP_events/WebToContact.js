 ...
 ********************************************************************************/
var grid2,grid3,grid4,grid3F,grid4F;var add_all_fields=SUGAR.language.get('app_strings','LBL_ADD_ALL_CONTACT_FIELDS');var remove_all_fields=SUGAR.language.get('app_strings','LBL_REMOVE_ALL_CONTACT_FIELDS');function addGrids(form_name){if(!check_form('WebToContactCreation')){return false;}
...
function checkFields(REQUIRED_CONTACT_FIELDS,CONTACT_SELECT_FIELDS){grid2=SUGAR_GRID_grid0;grid3=SUGAR_GRID_grid1;grid4=SUGAR_GRID_grid2;var reqFields='';for(var i=0;i<grid2.getRecordSet().getLength();i++){if(grid2.getRecord(i).getData()[2]!=null){reqFields=reqFields+grid2.getRecord(i).getData()[0]+', ';}}
if(reqFields){reqFields=reqFields.substring(0,reqFields.lastIndexOf(','));alert(REQUIRED_CONTACT_FIELDS+' '+reqFields);return false;}
else if(grid3.getRecordSet().getLength()==1&&grid4.getRecordSet().getLength()==1){alert(CONTACT_SELECT_FIELDS);return false;}
else{return true;}}
function askContactQ(direction,REQUIRED_CONTACT_FIELDS,CONTACT_SELECT_FIELDS){if(direction=='back'){var grid_Div=document.getElementById('grid_Div');var contact_Div=document.getElementById('contact_queries_Div');grid_Div.style.display='block';contact_Div.style.display='none';}
if(direction=='next'){if(!checkFields(REQUIRED_CONTACT_FIELDS,CONTACT_SELECT_FIELDS)){return false;}
else{var contact_Div=document.getElementById('contact_queries_Div');var grid_Div=document.getElementById('grid_Div');contact_Div.style.display='block';grid_Div.style.display='none';}}}
function eventPopulated(){var evt_populated=document.getElementById('event_id');if(evt_populated.value==0){return true;};return true;}
...
function displayAddRemoveDragButtons(Add_All_Fields,Remove_All_Fields){var addRemove=document.getElementById("contact_add_remove_button");if(grid2.getRecordSet().getLength()==0){addRemove.setAttribute('value',Remove_All_Fields);addRemove.setAttribute('title',Remove_All_Fields);}
