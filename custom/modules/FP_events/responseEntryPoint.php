...
if($type == 'accounts'){
    ...
    $event->load_relationship('accounts'); //get related targets

    if($response == 'accept'){
        //check to see if they have already responded to the email
        $check_q = 'SELECT email_responded FROM fp_events_accounts WHERE fp_event_id="'.$event->id.'" AND account_id="'.$delegate_id.'"';
        $check = $db->getOne($check_q);

        //update contact to accepted
        $query = 'UPDATE fp_events_accounts SET accept_status="Accepted", email_responded="1" WHERE fp_event_id="'.$event->id.'" AND account_id="'.$delegate_id.'" AND email_responded="0"';
        if($db->query($query) && $check != '1'){
        ...
        $check = $db->getOne($check_q);
        //update contact to accepted
        $query = 'UPDATE fp_events_accounts SET accept_status="Declined", email_responded="1" WHERE fp_event_id="'.$event->id.'" AND account_id="'.$delegate_id.'" AND email_responded="0"';
        if($db->query($query) && $check != '1'){
...
if($type == 'osy_contactspotentialmember'){

    $event->load_relationship('osy_contactspotentialmember'); //get related leads

    if($response == 'accept'){
        //check to see if they have already responded to the email
        $check_q = 'SELECT email_responded FROM fp_events_contactspotentialmember WHERE fp_event_id="'.$event->id.'" AND osy_contactspotentialmember_id="'.$delegate_id.'"';
        $check = $db->getOne($check_q);
        //update contact to accepted
        $query = 'UPDATE fp_events_contactspotentialmember SET accept_status="Accepted", email_responded="1" WHERE fp_event_id="'.$event->id.'" AND osy_contactspotentialmember_id="'.$delegate_id.'" AND email_responded="0"';
        if($db->query($query) && $check != '1'){
        ...
        $check = $db->getOne($check_q);
        //update contact to accepted
        $query = 'UPDATE fp_events_contactspotentialmember SET accept_status="Declined", email_responded="1" WHERE fp_event_id="'.$event->id.'" AND osy_contactspotentialmember_id="'.$delegate_id.'" AND email_responded="0"';
...
if($type == 'osy_other_contacts'){

    $event->load_relationship('osy_other_contacts'); //get related leads

    if($response == 'accept'){
        //check to see if they have already responded to the email
        $check_q = 'SELECT email_responded FROM fp_events_osy_other_contacts WHERE fp_event_id="'.$event->id.'" AND osy_other_contacts_id="'.$delegate_id.'"';
        $check = $db->getOne($check_q);
        //update contact to accepted
        $query = 'UPDATE fp_events_osy_other_contacts SET accept_status="Accepted", email_responded="1" WHERE fp_event_id="'.$event->id.'" AND osy_other_contacts_id="'.$delegate_id.'" AND email_responded="0"';
        if($db->query($query) && $check != '1'){
        ...
        $check = $db->getOne($check_q);
        //update contact to accepted
        $query = 'UPDATE fp_events_osy_other_contacts SET accept_status="Declined", email_responded="1" WHERE fp_event_id="'.$event->id.'" AND osy_other_contacts_id="'.$delegate_id.'" AND email_responded="0"';
