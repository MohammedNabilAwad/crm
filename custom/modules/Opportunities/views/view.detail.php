...
class CustomOpportunitiesViewDetail extends OpportunitiesViewDetail {
 	function CustomOpportunitiesViewDetail(){
 	...
 	function display() {
		...
		if(isset($v_record['id']) && !empty($v_record['id'])){
 			$this->dv->focus->osy_administration_panel_id=$v_record['id'];
 			$this->dv->focus->osy_view_taxes_payments=$v_record['view_taxes_payments'];
 			$this->dv->focus->osy_view_partial_payments=$v_record['view_partial_payments'];
 			$this->ev->focus->osy_view_training_package_amount=$v_record['view_training_package_amount'];
		} 		

		if(isset($this->dv->focus->leads_1_id) && $this->dv->focus->leads_1_id != ''){
			$q="SELECT account_name FROM leads WHERE id ='".$this->dv->focus->leads_1_id."' AND deleted = 0";
			$r=$db->query($q);
			$v=$db->fetchByAssoc($r);		
			$this->dv->focus->leads_1_name = $v['account_name'];
		}
