        ...
        if (!empty($alltabs)) {
            foreach ($alltabs as $key => $name) {
                if ($name == 'opportunities') {
                    if (($this->bean->member_type == 'Indirect Member')) {
                        $subpanel->subpanel_definitions->exclude_tab($name);
                    }
                }
                if ($name == 'leads') {
                    if (($this->bean->member_type == 'Association' || $this->bean->member_type == 'Indirect Member')) {
                        $subpanel->subpanel_definitions->exclude_tab($name);
                    }
                }
                if ($name == 'accounts_association_subpanel') {
                    if (($this->bean->member_type != 'Association')) {
                        $subpanel->subpanel_definitions->exclude_tab($name);
                    }
                }
            }
